...
| ------------------------------------------------------------------------- |
| Copyright ( c ) 2014 - 2017, Subin Siby. All Rights Reserved.             |
| ------------------------------------------------------------------------- |
...
HTML;
		...
		try {
			...
			if ( $this->config['features']['two_step_login'] === true && $this->loggedIn ) {
				...
				if ( ! isset( $_SESSION[$this->config['cookies']['names']['device_verified']] ) && ! isset( $_COOKIE[$this->config['cookies']['names']['device']] ) && $login_page === false ) {
					 ...
					 */
					$this->logout();
				} else if ( $this->config['two_step_login']['first_check_only'] === false ||
					(
	...
	public function forgotPassword() {
		...
		if ( ! isset( $_POST['logSysForgotPass'] ) && ! isset( $_GET['resetPassToken'] ) && ! isset( $_POST['logSysForgotPassChange'] ) ) {
		...
		} else if ( isset( $_GET['resetPassToken'] ) && ! isset( $_POST['logSysForgotPassChange'] ) ) {
			/**
		...
		} else if ( isset( $_POST['logSysForgotPassChange'] ) && isset( $_POST['logSysForgotPassNewPassword'] ) && isset( $_POST['logSysForgotPassRetypedPassword'] ) ) {
			...
			if ( $userID === false ) {
				...
				if ( empty( $_POST['logSysForgotPassNewPassword'] ) || empty( $_POST['logSysForgotPassRetypedPassword'] ) ) {
					...
					echo $this->getOutput( $curStatus );
				} else if ( $_POST['logSysForgotPassNewPassword'] !== $_POST['logSysForgotPassRetypedPassword'] ) {
		...
		} else if ( isset( $_POST['identification'] ) ) {
			/**
